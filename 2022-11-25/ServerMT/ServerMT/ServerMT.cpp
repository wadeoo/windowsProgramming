...
DWORD WINAPI ProcessRecvClientResponse(LPVOID lpParam)
{
	...
	socketForServer.mySend(*clientsocket, msg,NULL);
	
	while (TRUE)
	{
		char buffer[MAXBYTE] = { 0 };
		//recv(*clientsocket, buffer, MAXBYTE, NULL);
		socketForServer.myRecv(*clientsocket, buffer, NULL);
		if (strcmp(buffer, "退出") == 0)
		{
			char* exit_msg = "再见 \r\n";
			//send(*clientsocket, exit_msg, strlen(exit_msg) + sizeof(char), NULL);
			socketForServer.mySend(*clientsocket, exit_msg, NULL);
			printf("一个客户端已退出\n");
			break;
		}
		printf("\n客户端: %s\n", buffer);
	...
	socketForServer.myCloseSocket(*clientsocket);
	return 0;
}
...
DWORD WINAPI ProcessSendServerResponse(LPVOID lpParam)
{
	...
	while (TRUE)
	{
		...
		socketForServer.mySend(*clientsocket, mymsg, NULL);
	}
	//closesocket(*clientsocket);
	socketForServer.myCloseSocket(*clientsocket);
	return 0;
...
int main()
{
	...
	socketForServer.myBind((SOCKADDR*)&sockaddr);

	//listen(s, 1);
	socketForServer.myListen(1);
	...
	while (TRUE)
	{
		...
		while (!(clientsocket = socketForServer.myAccept(&clientAddr))){        //阻塞模式 直到有新的Tcp 接入
	...
	socketForServer.myCloseSocket(socketForServer.getMySocket());
