...
int ADO::InitConn(char* username, char* password, char* database){
	...
	//根据目前输入的数据，生成连接字符串
	if (!m_GenerateConnString(database)){
		printf("请输入ADO连接字符串");
		/*string conn;
		std::cin >> conn;
...
int ADO::GetTableInfo(char* tablename){//并将数据库中的信息读取到rs中
...
int ADO::Delete(char* table, char* tiaojian){
	_bstr_t sTable = table;
	_bstr_t sTiaojian = tiaojian;
	if (m_pRecordset != NULL && m_pRecordset->State){
		m_pRecordset->Close();
	}
	try
	{
		_bstr_t sql_delete = "delete from " + sTable + " where " + sTiaojian;
...
bool ADO::Update(char* table, char* neirong, char* tiaojian)
{
	_bstr_t sTable = table;
	_bstr_t sNeirong = neirong;
	_bstr_t sTiaojian = tiaojian;
	if (m_pRecordset != NULL && m_pRecordset->State){
		m_pRecordset->Close();
	}
	_bstr_t sql = "UPDATE [" + sTable + "] set " + sNeirong + " where " + sTiaojian;
	try
	{
...
int ADO::m_GenerateConnString(_bstr_t database){
	if (m_type == type_SqlSever)
	{
